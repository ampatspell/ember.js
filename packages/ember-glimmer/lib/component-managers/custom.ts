...
export interface DefinitionState<ComponentInstance = Opaque> {
  name: string;
...
export interface ComponentCapabilityMask {
  asyncLifeCycleCallbacks: boolean;
  destructor: boolean;
}
...
export interface ManagerDelegate<ComponentInstance = Opaque> {
  capabilities: ComponentCapabilityMask;
...
export interface ManagerDelegateWithAsyncLifeCycleCallbacks<ComponentInstance = Opaque>
...
export interface ManagerDelegateWithDestructors<ComponentInstance = Opaque>
  extends ManagerDelegate<ComponentInstance> {
...
*/
export default class CustomComponentManager<ComponentInstance = Opaque>
  extends AbstractComponentManager<CustomComponentState<ComponentInstance>, DefinitionState>
  implements
    WithStaticLayout<
      ...
      RuntimeResolver
    > {
  constructor(private delegate: ManagerDelegate<ComponentInstance>) {
    super();
  }
  ...
  update({ component, args }: CustomComponentState<ComponentInstance>) {
    this.delegate.updateComponent(component, valueForCapturedArgs(args));
  }

  didCreate({ component }: CustomComponentState<ComponentInstance>) {
    let { delegate } = this;
  ...
  didUpdate({ component }: CustomComponentState<ComponentInstance>) {
    let { delegate } = this;
  ...
  getContext({ component }: CustomComponentState<ComponentInstance>) {
  ...
  getDestructor(state: CustomComponentState<ComponentInstance>): Option<Destroyable> {
    if (hasDestructors(this.delegate)) {
      return state;
    } else {
      return null;
    }
  ...
  getLayout(state: DefinitionState): Invocation {
    return {
